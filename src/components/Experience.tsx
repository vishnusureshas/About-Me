...
import { Briefcase, Calendar, ArrowUpRight, Sparkles, Rocket } from "lucide-react"
import { experiences } from "@/lib/data"

export default function Experience() {
  return (
    <section className="relative py-20 sm:py-28" id="experience">
      <div className="absolute inset-0 bg-gradient-to-b from-background via-muted/18 to-background pointer-events-none" />
      <div className="absolute inset-0 animated-grid opacity-[0.45] pointer-events-none" />
      <div className="container mx-auto px-6 relative">
        ...
        <div className="max-w-5xl mx-auto relative">
          <div className="absolute left-4 md:left-1/2 top-6 bottom-6 w-px bg-gradient-to-b from-transparent via-border to-transparent hidden sm:block md:-translate-x-px" />
          <div className="absolute left-4 md:left-1/2 top-6 bottom-6 w-px bg-gradient-to-b from-primary/0 via-primary/40 to-secondary/0 blur-[1px] hidden sm:block md:-translate-x-px opacity-60" />
          ...
          <div className="space-y-8 sm:space-y-10">
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  <span className="hidden md:grid absolute left-1/2 top-8 -translate-x-1/2 w-3.5 h-3.5 rounded-full bg-background border-2 border-primary shadow-[0_0_14px_rgba(0,212,255,0.65)] place-items-center z-10">
                  ...
                  <div className={`hidden md:flex flex-1 ${isEven ? "justify-end" : "justify-start"} pt-2`}>
                    <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full glass text-xs font-medium premium-ring">
                    ...
                    <div className="group relative rounded-[24px] glass-strong p-6 sm:p-7 card-glow overflow-hidden hover:border-white/15 transition-colors">
                      ...
                      <div className="flex items-start gap-4">
                        <span className="w-11 h-11 rounded-xl bg-gradient-to-br from-primary to-secondary grid place-items-center flex-shrink-0 shadow-[0_8px_20px_rgba(0,212,255,0.28)] group-hover:scale-105 transition-transform">
                          {index === 0 ? <Rocket className="w-5 h-5 text-white" /> : <Briefcase className="w-5 h-5 text-white" />}
                        </span>
                        ...
                        <ul className="space-y-2.5">
                          {exp.responsibilities.map((item, i) => (
                            <li key={i} className="flex gap-3 text-sm leading-relaxed text-foreground/90">
                              <span className="mt-[9px] w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0 shadow-[0_0_8px_rgba(0,212,255,0.6)]" />
                              <span>{item}</span>
                            </li>
                          ))}
                        </ul>
                      ...
                      {exp.techStack && (
                        <div className="mt-6 flex flex-wrap gap-1.5">
                          {exp.techStack.map((tech) => (
                            <span key={tech} className="px-2.5 py-1 rounded-full bg-white/[0.06] border border-white/10 text-xs font-medium text-muted-foreground hover:text-foreground hover:border-primary/20 hover:bg-white/[0.08] transition-colors">
                              {tech}
                            </span>
                          ))}
                        </div>
